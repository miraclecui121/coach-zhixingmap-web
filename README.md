...
- 教练维护介绍、价格、可约时间、银行卡结算信息，并按月申请人工结算
...
微信支付按自然日切账，T 日交易通常在 T+1 进入资金账单和商户结算流程。商户平台自动提现可能在次日凌晨发起，最终到账时间以银行处理为准；教练月度结算应以商户资金到账后再人工打款。

## 资金和会计口径

当前平台是代收代付模型：学员支付全款进入平台微信支付商户号，资金结算到平台绑定银行卡后，平台再按订单账务给教练做人工银行卡结算。

- 学员付款成功时：记录平台代收资金；在服务未完成前，不应把全额都确认为平台收入。
- 服务完成且学员评价后：平台扣点确认为平台服务收入，教练部分确认为应付教练款。
- 给教练打款时：冲减应付教练款和银行存款。
- 平台扣点不再生成“提现单”，它是平台收入留存，不是第三方待付款。
